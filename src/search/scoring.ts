 ...
 */
export function decodeRedirectUrl(raw: string): string | undefined {
  try {
    ...
    if (wrapped) {
      ...
      if (wrapped.startsWith('a1')) {
        ...
        if (!candidate) candidate = decodeBase64(wrapped)
...
function tokensOf(query: string): string[] {
  ...
  const tokens: string[] = []
  for (const match of lower.matchAll(TOKEN_RE)) tokens.push(match[0])
  for (const char of lower) {
    if (/[\u4e00-\u9fff]/.test(char) && !tokens.includes(char)) tokens.push(char)
  }
  return tokens
}

/** Count how many query tokens appear in a text. */
function hits(text: string, tokens: string[]): number {
  const lower = text.toLowerCase()
  let count = 0
  for (const token of tokens) if (lower.includes(token)) count++
  return count
}
...
export function rankAndDedupe(
  ...
  const best = new Map<string, ScoredSource>()
  for (const source of sources) {
    ...
    const entry: ScoredSource = { ...source, url: realUrl, score }
    const existing = best.get(key)
    if (existing === undefined || entry.score > existing.score) best.set(key, entry)
  }
  ...
  for (const entry of sorted) {
    ...
    if (count >= maxDomainResults) continue
...
export function distinctUrlCount(sources: readonly RawSource[]): number {
