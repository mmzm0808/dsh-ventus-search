...
function pickBingMarket(query: string): string {
  return /[\u4e00-\u9fff]/.test(query) ? 'zh-CN' : 'en-US'
}

/** Parse Bing result blocks (<li class="b_algo">). */
function parseBingHtml(html: string, maxResults: number): RawSource[] {
  const out: RawSource[] = []
...
export async function bingSearch(query: string, opts: EngineSearchOptions): Promise<RawSource[]> {
  ...
  if (opts.maxResults > 0) params.set('count', String(Math.min(opts.maxResults, 20)))
...
function parseSo360Html(html: string, maxResults: number): RawSource[] {
  const out: RawSource[] = []
...
export async function so360Search(query: string, opts: EngineSearchOptions): Promise<RawSource[]> {
  const encoded = encodeURIComponent(query.trim())
  const variants = [
    'https://www.so.com/s?q=' + encoded,
    'https://www.so.com/s?ie=utf-8&q=' + encoded,
  ]
  let lastError: unknown = new Error('so360 search failed')
...
interface BiliSearchResponse {
  code?: number
  data?: { result?: BiliGroup[] }
}

/** Bilibili video search through the public search/all/v2 JSON API. */
export async function bilibiliSearch(query: string, opts: EngineSearchOptions): Promise<RawSource[]> {
  ...
  const parsed = JSON.parse(text) as BiliSearchResponse
  if (parsed.code !== 0) throw new Error('bilibili API error code ' + String(parsed.code))
  ...
  for (const item of items) {
    ...
    if (!item.arcurl || !item.title) continue
    let snippet = item.description ? cleanText(item.description) : undefined
    if (!snippet && item.author) snippet = item.author
    out.push({
      url: item.arcurl,
...
export const ENGINE_ADAPTERS: Record<EngineId, (query: string, opts: EngineSearchOptions) => Promise<RawSource[]>> = {
  bing: bingSearch,
  so360: so360Search,
