...
import type { EngineId, EngineSearchOptions, RawSource } from './engines.js'
...
function tooFewFloor(maxResults: number): number {
  return Math.min(3, maxResults)
}

/** LRU query cache with TTL (Map keeps insertion order; re-insert on hit). */
class QueryCache {
  private readonly entries = new Map<string, { at: number; result: WebSearchResult }>()
  ...
  set(key: string, result: WebSearchResult): void {
...
async function runPool<T>(items: readonly T[], worker: (item: T) => Promise<void>, concurrency: number): Promise<void> {
  let next = 0
...
function fallbackQueries(query: string): string[] {
  ...
  if (noQuotes !== query && noQuotes.trim().length > 0) variants.push(noQuotes.trim())
  const cnReduced = query
    .replace(/(什么是|如何|怎么|怎样|为什么|怎么样|的|了|吗|呢|和|与|及|在|是|请|帮我)/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
  if (cnReduced.length > 0 && cnReduced !== query && !variants.includes(cnReduced)) variants.push(cnReduced)
  const enReduced = query
    .replace(/\b(?:how|what|why|where|when|which|the|a|an|of|to|in|for|on|with)\b/gi, ' ')
    .replace(/\s+/g, ' ')
    .trim()
  if (enReduced.length > 0 && enReduced !== query && !variants.includes(enReduced)) variants.push(enReduced)
  return variants.slice(0, FALLBACK_QUERY_COUNT)
}

/** The Ventus search provider. */
export class VentusSearchProvider implements WebSearchProvider {
  readonly id = VENTUS_SEARCH_PROVIDER_ID
  private readonly cache = new QueryCache(100, 300_000)

  constructor(
    private readonly options: VentusSearchOptions,
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    if (this.options.cacheEnabled) this.cache.set(query, result)
    return result
  }
  ...
  private async runSearch(query: string, externalSignal?: AbortSignal): Promise<WebSearchResult> {
    ...
    try {
      ...
      if (externalAborted) throw new WebError('web search aborted', 'WEB_ABORTED')
      const ranked = rankAndDedupe(collected, query, this.options.maxDomainResults)
      const sources: WebSearchSource[] = ranked.slice(0, this.options.maxResults).map(entry => ({
        url: entry.url,
        title: entry.title,
  ...
  private engineOptions(query: string, signal: AbortSignal): EngineSearchOptions {
    return {
  ...
  private async collectPasses(
    enabled: Array<{ id: EngineId; run: (q: string, signal: AbortSignal) => Promise<RawSource[]> }>,
    query: string,
    ...
    if (distinctUrlCount(collected) >= floor || deadline.aborted) return
