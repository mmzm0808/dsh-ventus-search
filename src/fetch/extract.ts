...
function extractBetween(html: string, open: RegExp, close: RegExp): string | undefined {
...
function balancedExtent(html: string, startIndex: number, tag: string): string | undefined {
  ...
  for (;;) {
    ...
    if (depth <= 0) return html.slice(open.index, cursor)
  }
}

/** First element whose tag is article/main or carries role=main / a content class. */
function findContentContainer(html: string): string | undefined {
  ...
  while ((match = tagRe.exec(html)) !== null) {
    ...
    if (
      ...
      if (block !== undefined && textLength(block) >= MIN_READABLE_CHARS) return block
    }
  }
  return undefined
}
...
function pageTitle(html: string): string {
  ...
  if (title?.[1]) return decodeEntities(stripTags(title[1])).trim()
 ...
 */
export function extractReadable(html: string, url: string): WebFetchBody {
  const chromeFree = stripChrome(html)
  const container =
    extractBetween(chromeFree, /<article[\s>]/i, /<\/article>/i) ??
