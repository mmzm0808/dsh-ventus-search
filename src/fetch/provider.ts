...
function isBlocked(host: string, blockedDomains: readonly string[]): boolean {
...
function mirrorUrl(original: URL, mirrorHost: string): string {
...
export class VentusFetchProvider implements WebFetchProvider {
  readonly id = VENTUS_FETCH_PROVIDER_ID

  constructor(
    private readonly options: VentusFetchOptions,
  ...
  async fetch(request: WebFetchRequest, signal?: AbortSignal): Promise<WebFetchResult> {
    if (!this.available()) throw new WebError('ventus fetch disabled', 'WEB_PROVIDER_UNAVAILABLE')
  ...
  private async fetchOnce(target: string, signal?: AbortSignal): Promise<WebFetchResult> {
    ...
    try {
      ...
      const body: WebFetchBody = isHtml ? extractReadable(capped, finalUrl) : { kind: 'text', content: capped }
      return {
        url: finalUrl,
      ...
      if (signal?.aborted) throw new WebError('web fetch aborted', 'WEB_ABORTED', { cause: error })
      if (timedOut) throw new WebError('ventus fetch timed out', 'WEB_FETCH_TIMEOUT', { cause: error })
...
function safeCharset(charset: string): string {
  try {
